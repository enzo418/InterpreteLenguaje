...
bool TemplateAutomata(std::istream& fuente, ulong& control, std::string& lexema,
    ...
    // -- Verificamos que el estado en el cual estamos no sea invalido y no sea
    // el fin del archivo
    while (estadoActual != estadoMuerto && !fuente.eof()) {
        int simb = CarASimbolo(c);
        estadoActual = delta[{estadoActual, simb}];

        // -- Si no esta en un estado muerto el caracter es valido, por lo que
        // lo agregamos
    ...
    // -- Si llegamos al final del archivo limpiar el bit de EOF, asi poder
    // utilizar seekg si es necesario
    if (fuente.eof()) fuente.clear();

    // -- La cadena obtenida es valida si el ultimo estado que fue valido antes
    // de que fallara es estado final
    if (ExisteEn(finales, ultimoEstado)) {
        control += lexema.size();
        fuente.seekg(
            control);  // Cambiamos la posicion del cursor a la anterior a la
                       // actual (la que fallo el automata)
        esValida = true;
...
bool EsIdentificador(std::istream& fuente, ulong& control,
                     std::string& lexema) {
...
bool EsConstanteReal(std::istream& fuente, ulong& control,
                     std::string& lexema) {
    // -- Definimos el alfabeto de entrada, especial = _
    enum Sigma { Digito, Signo, Punto, Exponente, Otro };
    ...
    Delta delta = {
        //		 q	 x	simbolo		->	q
        {{0, Exponente}, 7}, {{0, Digito}, 2}, {{0, Signo}, 1},
        {{0, Punto}, 7},     {{0, Otro}, 7},

        {{1, Exponente}, 7}, {{1, Digito}, 2}, {{1, Signo}, 7},
        {{1, Punto}, 7},     {{1, Otro}, 7},

        {{2, Exponente}, 5}, {{2, Digito}, 2}, {{2, Signo}, 7},
        {{2, Punto}, 3},     {{2, Otro}, 7},

        {{3, Exponente}, 5}, {{3, Digito}, 4}, {{3, Signo}, 7},
        {{3, Punto}, 7},     {{3, Otro}, 7},

        {{4, Exponente}, 5}, {{4, Digito}, 4}, {{4, Signo}, 7},
        {{4, Punto}, 7},     {{4, Otro}, 7},

        {{5, Exponente}, 7}, {{5, Digito}, 8}, {{5, Signo}, 6},
        {{5, Punto}, 7},     {{5, Otro}, 7},

        {{6, Exponente}, 7}, {{6, Digito}, 8}, {{6, Signo}, 7},
        {{6, Punto}, 7},     {{6, Otro}, 7},

        {{7, Exponente}, 7}, {{7, Digito}, 7}, {{7, Signo}, 7},
        {{7, Punto}, 7},     {{7, Otro}, 7},

        {{8, Exponente}, 7}, {{8, Digito}, 8}, {{8, Signo}, 7},
        {{8, Punto}, 7},     {{8, Otro}, 7},
    };
    ...
    const ushort q0 = 0;
    bool esConstReal = TemplateAutomata(fuente, control, lexema, CarASimbolo,
                                        delta, finales, q0, estadoMuerto);
...
bool EsConstanteEntera(std::istream& fuente, ulong& control,
    ...
    Delta delta = {
        //		 q	 x	simbolo	 ->	q
        {{0, Signo}, 1}, {{0, Digito}, 2}, {{0, Otro}, 3},

        {{1, Signo}, 3}, {{1, Digito}, 2}, {{1, Otro}, 3},

        {{2, Signo}, 3}, {{2, Digito}, 2}, {{2, Otro}, 3},

        {{3, Signo}, 3}, {{3, Digito}, 3}, {{3, Otro}, 3},
    };
    ...
    bool esConstEntera = TemplateAutomata(fuente, control, lexema, CarASimbolo,
                                          delta, finales, q0, estadoMuerto);
...
bool EsOperadorRelacional(std::istream& fuente, ulong& control,
                          std::string& lexema) {
    // -- Definimos el alfabeto de entrada
    enum Sigma { Mayor, Menor, Igual, Otro };
    ...
    Delta delta = {
        //		 q	 x	simbolo	 ->	q
        {{0, Menor}, 1}, {{0, Mayor}, 4}, {{0, Igual}, 3}, {{0, Otro}, 5},

        {{1, Menor}, 5}, {{1, Mayor}, 2}, {{1, Igual}, 2}, {{1, Otro}, 5},

        {{2, Menor}, 5}, {{2, Mayor}, 5}, {{2, Igual}, 5}, {{2, Otro}, 5},

        {{3, Menor}, 5}, {{3, Mayor}, 5}, {{3, Igual}, 2}, {{3, Otro}, 5},

        {{4, Menor}, 5}, {{4, Mayor}, 5}, {{4, Igual}, 2}, {{4, Otro}, 5},

        {{5, Menor}, 5}, {{5, Mayor}, 5}, {{5, Igual}, 5}, {{5, Otro}, 5}};
    ...
    bool esOperadorRelacional = TemplateAutomata(
        fuente, control, lexema, CarASimbolo, delta, finales, q0, estadoMuerto);
...
bool EsCadena(std::istream& fuente, ulong& control, std::string& lexema) {
    ...
    Delta delta = {//		 q	 x	simbolo	 ->	q
                   {{0, Comilla}, 1}, {{0, Car}, 3}, {{0, Otro}, 3},

                   {{1, Comilla}, 2}, {{1, Car}, 1}, {{1, Otro}, 3},

                   {{2, Comilla}, 3}, {{2, Car}, 3}, {{2, Otro}, 3},

                   {{3, Comilla}, 3}, {{3, Car}, 3}, {{3, Otro}, 3}};
    ...
    bool esCadena = TemplateAutomata(fuente, control, lexema, CarASimbolo,
                                     delta, finales, q0, estadoMuerto);
...
bool EsSimboloEspecial(std::istream& fuente, ulong& control,
                       std::string& lexema, ComponenteLexico& complex) {
    char c = '\0';
    bool esSimbolo = true;
    fuente.get(c);
    ...
    if (!esSimbolo)
        fuente.seekg(control);  //// Si no es simbolo, volver el cursor a la pos
                                ///de la cual obtuvimos el caracter
    else {
        lexema = c;
        complex = cl;
    }
