...
bool ObtenerSiguienteComplex(std::istream& fuente, ulong& control,
                             ComponenteLexico& complex, std::string& lexema,
                             TablaSimbolos& ts) {
    /** funcion  |  Utilidad
     *
     *  peek()      Devuelve el caracter sobre el cual esta el cursor del
     * archivo, sin removerlo del buffer. get()       Obtiene y devuelve el
     * caracter sobre el cual esta el cursor del archivo. eof()       Verifica
     * si es el final del archivo tellg()     Devuelve la posicion actual del
     * puntero|control|cursor archivo.
     */
