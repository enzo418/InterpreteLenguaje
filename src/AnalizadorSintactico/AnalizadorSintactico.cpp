...
int ObtenerArbolDerivacion(std::ifstream &fuente, Nodo *arbol, TAS &tas,
  ...
  AnalizadorLexico::ComponenteLexico complex =
      AnalizadorLexico::ComponenteLexico::Id;
  ...
  while (!exito && !error) {
    ...
    if (EsVariable(X)) {
      ...
      if (!produccion.empty()) {
        ...
        // cambiamos el tamaño del vector para asegurarnos al menos sz
        ...
        for (int i = sz - 1; i >= 0; i--) {
          /**
           * strcmp(x,y): Compara la longitud de la cadena x con la longitud de
           * la cadena y
           */
          ...
          if (strcmp(produccion[i], "epsilon") != 0)
            pilaSimbolos.push(
                std::pair<const char *, Nodo *>(produccion[i], nodo));
        }
      } else {
        error = true;
        ...
        if (pilaSimbolos.size() > 2) {
          ...
          mensajeError =
              "Se esperaba \"" + _F + "\", se obtuvo \"" + lexema + "\"";
        } else
      ...
      if (volcar) {
        std::cout << "\nDesapilado Terminal: \n\t"
                  << "X = " << X << " | lexema = " << lexema
                  << " | complex iguales? "
                  << (StringAComplex(X) == complex ? "si" : "no") << std::endl;
      }
      ...
      if (StringAComplex(X) == complex) {
        ...
        // si no se pudo obtener el siguiente complex significa que llegamos al
        // final del archivo
