    ...
    while (!exito && !error) {
        ...
        if (EsVariable(X)) {
            ...
            if (!produccion.empty()) {
                ...
                // apilar todos los simbolos (de derecha a izquierda) y crear
                // sus nodos
                for (int i = sz - 1; i >= 0; i--) {
                    /**
                     * strcmp(x,y): Compara la longitud de la cadena x con la
                     * longitud de la cadena y
                     */
                    ...
                    // Si es epsilon va en el arbol pero no en la pila de
                    // simbolos.
            ...
            if (volcar) {
                std::cout << "\nDesapilado Terminal: \n\t"
                          << "X = " << X << " | lexema = " << lexema
                          << " | complex iguales? "
                          << (StringAComplex(X) == complex ? "si" : "no")
                          << std::endl;
            }
            ...
            if (StringAComplex(X) == complex) {
                ...
                // si no se pudo obtener el siguiente complex significa que
